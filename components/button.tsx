...
const getButtonStyle = (level: string) => {
    if (level === 'main') {
        ...
        return "bg-error-3 text-neutral-1 hover:bg-error-3-5 hover:text-white hover:ring-error-2 hover:ring-2"
    }
}
export default function Button({className, level, ...props} : React.ComponentProps<"button"> & { level: string }) {
    return <button
        className={classNames(
            "rounded-xl cursor-pointer p-2 pl-5 pr-5 text-center",
