...
export default function PasswordInput({ className, labelClassName, label, error, ...props }: React.ComponentProps<"input"> & { label?: string, labelClassName?: string, error?: string }) {
    ...
    return <div className={classNames("group relative", labelClassName)}>
        <label>{label}
            <div className={classNames(
                "rounded-lg border border-neutral-3 text-md mb-1 pr-4 h-9 w-full flex",
                ...
                <input
                    className="border-none outline-none p-1 pl-3 w-full"
                    type={visible ? 'input' : 'password'}
                    {...props}
                />
                <button type='button' className="invisible group-hover:visible w-5 cursor-pointer" onClick={() => setVisible(v => !v)}>
                    {visible ?
